...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('codeShiftAIView', chatViewProvider));
    ...
    const completionProvider = new completionProvider_1.CodeShiftCompletionProvider();
    ...
    let explainCodeCommand = vscode.commands.registerCommand('codeShiftAI.explainCode', async () => {
        ...
        vscode.window.withProgress({
            ...
            try {
                const explanation = await apiService.sendChatMessage(`Explain this code: ${selectedText}`);
...
class CodeShiftAIChatViewProvider {
    _view;
    _context;
    constructor(context) {
...
function getChatWebviewContent(iconPath) {
    return `<!DOCTYPE html>
	<html lang="en">
	<head>
		<meta charset="UTF-8">
...
function handleUserMessage(webview, text) {
